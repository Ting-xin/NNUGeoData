...
const addFolder = (folderName) => {
  ...
  createCatalog({
    userId: user.id,
    parentId: catalog.id,
    name: folderName ? folderName : "新建文件夹",
  })
    .then((res) => {
      ElMessage({
        message: "新建文件夹成功",
      ...
      store.commit("catalog/setDataList", list);
    })
    .catch((err) => {
      ElMessage({
        message: "新建文件夹失败",
...
const handleDelete = (index, row) => {
  if (row.type === "file") {
    deleteFile({ id: row.id, catalogId: catalogId })
      .then((res) => {
        console.log("data.value: ", props.list.value);
        this.props.list.splice(index, 1);
        console.log("data.value: ", props.list.value);

        store.commit("catalog/setDataList", props.list);
        ElMessage({
          message: "删除文件成功",
          type: "success",
        });
